...
using a linear model on top of tweet2vec embeddings.
"""
...
def extend_np_by_four(np_in):
    np_temp = np.zeros((4 * np_in.shape[0], np_in.shape[1]))
    for i in xrange(np_in.shape[0]):
        for j in xrange(5):
            np_temp[4 * i: 4 * i + j] = np_in[i]
    return np_temp
...
if __name__ == '__main__':
    ...
    ### Data specific to tweet2vec ###
    context_t2vembedding_file = './embeddings_contexts.npy'
    gtresponses_t2vembedding_file = './embeddings_gtresponses.npy'
    modelresponses_t2vembedding_file = './embeddings_modelresponses.npy'

    context_emb = np.load(context_t2vembedding_file)
    gtresponse_emb = np.load(gtresponses_t2vembedding_file)
    modelresponse_emb = np.load(modelresponses_t2vembedding_file)

    context_emb = extend_np_by_four(context_emb)
    gtresponse_emb = extend_np_by_four(gtresponse_emb)
    
    emb_dim = context_emb.shape[1]
    twitter_dialogue_embeddings = np.zeros((context_emb.shape[0], 3, emb_dim))

    for i in range(context_emb.shape[0]):
        twitter_dialogue_embeddings[i, 0, :] =  context_emb[i]
        twitter_dialogue_embeddings[i, 1, :] =  gtresponse_emb[i]
        twitter_dialogue_embeddings[i, 2, :] =  modelresponse_emb[i]
    ...
    if use_aux_features:
        ...
        aux_features = np.zeros((context_emb.shape[0], 5))
    ...
    l1reg_list = [0.005, 0.01, 0.02]#1e-3, 1e-2, 0.1]
